  ...
  return (
    <div className="flex flex-col gap-5 w-full max-w-[1600px] mx-auto -mt-1 sm:-mt-2 pb-24 md:pb-8">
      ...
      <Dialog open={Boolean(selectedCategoryModal)} onOpenChange={(open) => { if (!open) setSelectedCategoryModal(null) }}>
        <DialogContent className="max-w-2xl p-6 sm:p-7 bg-[#18181B] border border-white/10 text-white rounded-3xl max-h-[88vh] overflow-y-auto shadow-2xl">
          {viewTransactionsCat && (() => {
            const ModalCategoryIcon = getCategoryIcon(viewTransactionsCat.category)
            const hasLimit = viewTransactionsCat.monthly_limit > 0
            ...
            return (
              <div className="flex flex-col gap-5">
                <DialogHeader className="p-0 text-left">
                  <div className="flex justify-between items-start gap-4 pr-6">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-10 h-10 rounded-2xl bg-zinc-900 border border-white/10 flex items-center justify-center shrink-0 text-white shadow-inner">
                        <ModalCategoryIcon className="w-5 h-5 text-zinc-300" />
                      </div>
                      <div className="flex flex-col min-w-0">
                        <DialogTitle className="text-base sm:text-lg font-bold text-white tracking-tight truncate">
                          {t.categories[viewTransactionsCat.category] || viewTransactionsCat.category}
                        </DialogTitle>
                        <span className="text-xs text-zinc-400">
                          {viewTransactionsCat.transactions.length} {language === "fr" ? `opération${viewTransactionsCat.transactions.length > 1 ? "s" : ""}` : `transaction${viewTransactionsCat.transactions.length > 1 ? "s" : ""}`} • {periodMode === "last_30_days" ? t.budgets.last30Days : formatMonthName(selectedMonth)}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col items-end shrink-0">
                      ...
                      {hasLimit ? (
                        <span className="text-[11px] font-mono text-zinc-400">
                          / {formatAmount(viewTransactionsCat.monthly_limit)} ({viewTransactionsCat.percentage}%)
                        </span>
                      ) : (
                        <button
                          type="button"
                          onClick={() => {
                            ...
                            handleOpenSetBudget(viewTransactionsCat.category, 0)
                          }}
                          className="text-[11px] text-indigo-400 hover:text-indigo-300 font-semibold cursor-pointer hover:underline pt-0.5"
                        >
                          + {t.budgets.define}
                        </button>
                      )}
                ...
                <div className="flex flex-col divide-y divide-white/5 border border-white/10 rounded-2xl bg-zinc-950/70 overflow-hidden">
                  {viewTransactionsCat.transactions.length === 0 ? (
                    <div className="py-12 px-6 text-center text-xs text-zinc-500">
                    ...
                    viewTransactionsCat.transactions.map((tx) => {
                      ...
                      return (
                        <div
                          ...
                          className={`p-3.5 sm:px-4 sm:py-3.5 flex items-center justify-between gap-3 transition-colors cursor-pointer group ${
                            isExcluded
                              ? "bg-zinc-950/40 opacity-60 hover:opacity-100 hover:bg-white/[0.03]"
                              : "hover:bg-white/[0.04]"
                          }`}
                        >
                          {/* Left Info: Merchant, Date, Subcategory Badge */}
                          <div className="flex flex-col min-w-0 flex-1 pr-2">
                            <div className="flex items-center gap-2 flex-wrap">
                              <span className={`text-xs sm:text-sm font-semibold group-hover:text-indigo-300 transition-colors truncate ${
                                isExcluded ? "line-through text-zinc-400" : "text-white"
                              }`}>
                                {tx.merchant || tx.raw_label || "Dépense"}
                              </span>
                              {isExcluded && (
                                <span className="text-[10px] font-medium py-0.5 px-2 rounded-md border border-amber-500/30 bg-amber-500/10 text-amber-300 shrink-0">
                                  {language === "fr" ? "Exclue du budget" : "Excluded"}
                                </span>
                              )}
                            </div>
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              ...
                              {tx.subcategory && (
                                <span className="text-[10px] text-zinc-400 font-medium px-2 py-0.5 rounded-md bg-white/5 border border-white/5">
                                  {t.categories[tx.subcategory] || tx.subcategory}
                                </span>
                              )}
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-2.5 shrink-0" onClick={(e) => e.stopPropagation()}>
                            ...
                            <button
                              ...
                              className="p-1.5 sm:p-2 text-zinc-500 hover:text-white hover:bg-white/10 rounded-xl transition-all cursor-pointer"
                            >
      ...
      <Dialog open={isExcludedListModalOpen} onOpenChange={setIsExcludedListModalOpen}>
        <DialogContent className="max-w-xl p-6 bg-[#18181B] border border-white/10 text-white rounded-3xl max-h-[85vh] overflow-y-auto shadow-2xl">
              ...
              {allMonthExcludedTransactions.length === 0 ? (
                ...
                allMonthExcludedTransactions.map((tx) => {
                  ...
                  return (
                    <div
                      key={tx.id}
                      className="p-3.5 flex justify-between items-center hover:bg-white/5 transition-colors"
                    >
            ...
            <form onSubmit={handleSaveTxCategory} className="flex flex-col gap-4">
              ...
              <div className="p-3.5 rounded-2xl bg-zinc-900/80 border border-white/5 flex items-center justify-between">
                <div className="flex flex-col min-w-0 pr-2">
                  <span className="text-xs font-bold text-white truncate">
                    {editingTx.merchant || editingTx.raw_label || "Dépense"}
                  </span>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <select
                  ...
                  className="bg-zinc-900 border border-white/10 rounded-xl px-3 py-2.5 text-xs text-white outline-none cursor-pointer focus:border-white/20 transition-colors"
                >
                  {categoriesList.map((cat, idx) => (
                    <option key={cat.id || cat.name || `edit-cat-${idx}`} value={cat.name} className="bg-[#18181B] text-white">
                      {t.categories[cat.name] || cat.name}
                    </option>
                  ))}
                </select>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                {availableSubcategories.length > 0 ? (
                  <select
                    ...
                    className="bg-zinc-900 border border-white/10 rounded-xl px-3 py-2.5 text-xs text-white outline-none cursor-pointer focus:border-white/20 transition-colors"
                  >
                    <option value="" className="bg-[#18181B] text-zinc-400">{language === "fr" ? "Aucune sous-catégorie" : "No subcategory"}</option>
                    {availableSubcategories.map((sub, idx) => (
                      <option key={`sub-${sub}-${idx}`} value={sub} className="bg-[#18181B] text-white">
                  ...
                  <Input
                    ...
                    value={editSubcategory}
                    onChange={(e) => setEditSubcategory(e.target.value)}
                    className="bg-zinc-900 border-white/10 text-white text-xs h-10 rounded-xl focus:border-white/20"
                  />
                )}
              </div>

              {/* Auto rule checkbox */}
              <label className="flex items-start gap-2.5 p-3 rounded-2xl bg-white/[0.02] border border-white/5 cursor-pointer hover:bg-white/[0.04] transition-colors">
                <input
                  type="checkbox"
              ...
              <label className="flex items-start gap-2.5 p-3 rounded-2xl bg-amber-500/5 border border-amber-500/15 cursor-pointer hover:bg-amber-500/10 transition-colors">
                <input
                  type="checkbox"
              ...
              <div className="flex items-center gap-2 pt-2">
                <Button
                  type="submit"
                  disabled={isSavingTx}
                  className="flex-1 bg-white hover:bg-zinc-200 text-zinc-950 text-xs h-10 rounded-xl font-bold cursor-pointer shadow-md transition-all gap-1.5"
                >
                  {isSavingTx ? (
                    <>
                      <RefreshCw className="w-3.5 h-3.5 animate-spin" />
                ...
                <Button
                  ...
                  className="text-zinc-400 hover:text-white hover:bg-white/5 text-xs h-10 px-4 rounded-xl cursor-pointer"
                >
                  {t.common.cancel}
                </Button>
              </div>
