...
export function UpdateModal({ isOpen, onClose, updateInfo }: UpdateModalProps) {
  ...
  const handleStartUpdate = async () => {
    try {
      setIsUpdating(true)
      await FinlyAPI.startUpdateDownload()
      ...
      pollIntervalRef.current = setInterval(async () => {
        try {
          ...
          if (status.status === "ready") {
            if (pollIntervalRef.current) clearInterval(pollIntervalRef.current)
            // Launch installer, close app and restart
          ...
          {updateInfo.is_container ? (
            ...
            <div className="space-y-3">
              <Button
                ...
                disabled={isUpdating}
                className="w-full bg-indigo-600 hover:bg-indigo-500 disabled:opacity-75 text-white text-xs font-semibold py-2.5 rounded-xl shadow-md shadow-indigo-600/20 transition-all cursor-pointer flex items-center justify-center gap-2"
              >
                {isUpdating ? (
                  <>
                    <Loader2 className="w-3.5 h-3.5 animate-spin text-white" />
                    <span>Lancement de l'installateur...</span>
                  </>
                ) : (
                  <>
