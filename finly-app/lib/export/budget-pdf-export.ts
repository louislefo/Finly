...
export const PDF_STRINGS = {
  fr: {
    ...
    footerConfidential: "Document confidentiel généré automatiquement par Finly",
  ...
  en: {
    ...
    footerConfidential: "Confidential document automatically generated by Finly",
...
export function exportBudgetToPdf({
  ...
  doc.setFont("helvetica", "bold")
  doc.setFontSize(15)
  doc.text(strings.reportTitle, margin + 23, y + 5.5)
  ...
  doc.text(`${strings.ownerLabel} ${resolvedUserName}`, pageWidth - margin, y + 1.5, { align: "right" })
  doc.text(`${strings.periodLabel} ${periodName}`, pageWidth - margin, y + 5.2, { align: "right" })
  doc.text(`${strings.accountLabel} ${resolvedAccountName}`, pageWidth - margin, y + 8.9, { align: "right" })
  doc.text(`${strings.editedLabel} ${nowStr}`, pageWidth - margin, y + 12.6, { align: "right" })
  ...
  doc.setFont("helvetica", "bold")
  doc.setFontSize(8.5)
  doc.text(strings.budgetSectionTitle, margin + 4, y + 4.5)
  ...
  doc.text(strings.colCategory, margin + 3, y + 3.8)
  doc.text(strings.colAllocated, margin + 60, y + 3.8, { align: "right" })
  doc.text(strings.colSpent, margin + 95, y + 3.8, { align: "right" })
  doc.text(strings.colRemaining, margin + 130, y + 3.8, { align: "right" })
  doc.text(strings.colUsed, margin + 152, y + 3.8, { align: "right" })
  doc.text(strings.colStatus, pageWidth - margin - 3, y + 3.8, { align: "right" })
  ...
  sortedItems.forEach((it, idx) => {
    ...
    doc.setFont("helvetica", "normal")
    doc.setFontSize(7)
    doc.text(getCatLabel(it.category), margin + 3, y + 3.5)
    ...
    if (limit > 0) {
      if (it.spent > limit) {
        ...
        doc.setFont("helvetica", "bold")
        doc.text(strings.statusExceeded, pageWidth - margin - 3, y + 3.5, { align: "right" })
      } else if (it.percentage >= 85) {
        doc.setTextColor(217, 119, 6)
        doc.setFont("helvetica", "bold")
        doc.text(strings.statusWarning, pageWidth - margin - 3, y + 3.5, { align: "right" })
      } else {
        doc.setTextColor(5, 150, 105)
        doc.text(strings.statusConform, pageWidth - margin - 3, y + 3.5, { align: "right" })
      }
    } else {
      doc.setTextColor(148, 163, 184)
      doc.text(strings.statusUnbudgeted, pageWidth - margin - 3, y + 3.5, { align: "right" })
    }
  ...
  if (topExpenses.length > 0) {
    ...
    topExpenses.forEach((tx, idx) => {
      ...
      doc.setTextColor(100, 116, 139)
      doc.text(getCatLabel(tx.category || strings.fallbackCategory), margin + 115, y + 3.3)
